...
export class AnimalListComponent implements OnInit {
  ...
  appliedSearch = signal<string>(''); // คำที่กดค้นหาแล้วจริง
  ...
  // ✅ โหลดสัตว์: รองรับทั้งแบบมี pagination และไม่มี + รองรับ search
  loadAnimals(url?: string) {
    // ถ้ากด next/prev มาจาก backend (url มีอยู่แล้ว) ก็ใช้ url นั้นเลย
    // แต่ถ้า url ถูกส่งมาแบบเต็มๆ แล้ว search/page จะถูก backend ใส่มาให้เอง
    if (url) {
    ...
    const baseUrl = `${this.API_BASE}/api/animals/`;
    const page = this.currentPage();

    const params: string[] = [`page=${page}`];

    // ✅ ส่ง search ไป backend ถ้ามี
    const q = this.appliedSearch().trim();
    if (q) params.push(`search=${encodeURIComponent(q)}`);

    const targetUrl = `${baseUrl}?${params.join('&')}`;

    this.http.get(targetUrl, { headers: this.authHeaders() }).subscribe({
      next: (data: any) => {
